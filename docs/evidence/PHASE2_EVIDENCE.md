...
- Merged Studio commit `f8f28f9` extends the journey in run `34633623345`: Chromium publishes the validated draft, observes the immutable owner-revision outcome (or the idempotent already-published outcome), returns to Library, refreshes the live catalog, and sees the published definition. This proves browser publication and published-definition reload, but not browser revocation, plan inspection, or artifact retrieval.
- Merged Studio commit `418100e` extends the journey in run `34635353603`: after owner-backed autosave, Chromium returns to Library, reopens the definition, and observes “Loaded the owner-backed draft.” This proves browser draft hydration from the persisted owner record, but not browser conflict handling.
- Merged Studio commit `062267e` extends the journey in run `34636278029`: a second authenticated owner save advances the persisted revision, then a stale browser JSON edit autosaves and receives the owner conflict record. Chromium renders “Local and remote drafts diverged” with the remote-reload action. This proves browser draft-save conflict presentation, but not retry, merge application, or publication-conflict handling.
- Merged Studio commit `c6d8cf2` extends the journey in run `34636988818`: Chromium selects “Reload remote” from that real conflict and returns to the saved owner-backed revision. This proves browser remote-conflict resolution, but not retry, merge application, or publication-conflict handling.
...
The live adapter is real code against the merged owner routes, including draft persistence, conditional save, capability validation, and publication. A live synthetic process completed create, save, validate, publish, and reload checks. GitHub-hosted Chromium evidence now covers authenticated browser pairing, live-mode selection, draft creation, autosave, persisted-draft hydration, draft-save conflict presentation and remote resolution, validation, publication, and published-definition reload. Browser session revocation, plan inspection, artifact retrieval, conflict retry or merge application, and publication-conflict handling remain unavailable. Fixture mode is explicit and deterministic; its evidence cannot satisfy native gameplay acceptance.
