...
The initial Chromium command passed both browser journeys (2/2) against the built preview. The 2026-09-11 continuation added unsupported-import archival retention and passed all three Studio browser journeys in Chromium and Firefox. GitHub Actions also passed Chromium, Firefox, and WebKit. Merged Studio commits `b7347fc`, `257ccec`, `f64efdf`, `02ecd93`, `f8f28f9`, and `418100e` add a GitHub-hosted authenticated live-owner Chromium regression: it builds the pinned harness, starts it with a bearer token, verifies owner health, enters the token and subject in the browser, activates live-owner mode, creates a draft, observes the owner-backed autosave, reopens and hydrates the persisted draft, receives the owner validation digest, publishes the immutable revision, refreshes the Library, and reloads the published definition. It does not yet cover revocation, plans, artifacts, or browser conflict handling.
