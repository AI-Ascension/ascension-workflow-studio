...
const admittedRoutes = [
  ...
  ["GET", "/v1/workflow-runs/:id/provider-session-policy", "admitted"],
  ["POST", "/v1/workflow-runs/:id/provider-session-policy/import", "admitted"],
  ["POST", "/v1/workflow-runs/:id/provider-session-policy/proposals/:id", "admitted"],
  ["POST", "/v1/workflow-runs/:id/provider-session-policy/proposals/:id/approve", "admitted"],
  ["POST", "/v1/workflow-runs/:id/provider-session-policy/proposals/:id/adopt", "admitted"],
  ["POST", "/v1/workflow-runs/:id/provider-session-policy/adoptions", "admitted"],
...
export function CompatibilityView({ mode, onModeChange, liveClient, linkMappings, onAddMapping, onRemoveMapping }: CompatibilityViewProps): JSX.Element {
  ...
  return <section className="view-stack" aria-labelledby="compatibility-title">
    ...
    <div className="settings-grid">
      ...
      <section className="panel-card"><div className="panel-title"><div><p className="eyebrow">Session credential</p><h2>Pairing token</h2></div><StatusBadge tone="muted">memory only</StatusBadge></div><label className="field-label">Bearer token<input type="password" value={token} onChange={(event) => setToken(event.target.value)} placeholder="Paste for this tab only" autoComplete="off" /></label><label className="field-label">Authenticated actor subject<input value={actorScope} onChange={(event) => setActorScope(event.target.value)} placeholder="profile:studio" autoComplete="off" /><span className="field-help">Must match the subject bound to the owner token; it is sent only on control commands. Policy reads need workflow:read; changes need workflow:control, and uploads also need workflow:content:write.</span></label><div className="control-grid"><button className="button button-secondary" onClick={() => void checkConnection()} disabled={connection === "checking"}>{connection === "checking" ? "Checking…" : "Check owner connection"}</button><button className="button button-quiet" onClick={() => { setToken(""); setActorScope("profile:studio"); liveClient.setToken(undefined); liveClient.setActorScope(undefined); setConnection("idle"); setMessage("Session material cleared from this tab."); }}>Clear session</button></div>{message ? <p className={`connection-message connection-${connection}`}>{message}</p> : null}</section>
    </div>
      <section className="panel-card"><div className="panel-title"><div><p className="eyebrow">Phase 1 admission</p><h2>Owner route matrix</h2></div><span className="muted">source: merged sts2-harness at a3bccbe</span></div><p className="muted">Saved provider-session policy routes are available in merged Harness revision <code>a3bccbec18cdd7afe7120c2332168c69de312cf0</code>. This admits the authenticated metadata owner API; it does not claim live provider execution.</p><div className="route-table" role="table" aria-label="Phase 1 owner routes"><div className="route-row route-header" role="row"><span>Method</span><span>Path</span><span>Studio use</span></div>{admittedRoutes.map(([method, path, state]) => <div className="route-row" role="row" key={`${method}-${path}`}><code>{method}</code><code>{path}</code><StatusBadge tone={state === "admitted" ? "success" : "warning"}>{state}</StatusBadge></div>)}</div></section>
