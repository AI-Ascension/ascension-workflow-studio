...
export function CompatibilityView({ mode, onModeChange, liveClient }: CompatibilityViewProps): JSX.Element {
  ...
  return <section className="view-stack" aria-labelledby="compatibility-title">
    ...
    <div className="settings-grid">
      ...
      <section className="panel-card"><div className="panel-title"><div><p className="eyebrow">Session credential</p><h2>Pairing token</h2></div><StatusBadge tone="muted">memory only</StatusBadge></div><label className="field-label">Bearer token<input type="password" value={token} onChange={(event) => setToken(event.target.value)} placeholder="Paste for this tab only" autoComplete="off" /></label><button className="button button-secondary" onClick={() => void checkConnection()} disabled={connection === "checking"}>{connection === "checking" ? "Checking…" : "Check owner connection"}</button>{message ? <p className={`connection-message connection-${connection}`}>{message}</p> : null}</section>
    </div>
