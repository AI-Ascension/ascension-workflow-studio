...
export function CompatibilityView({ mode, onModeChange, liveClient }: CompatibilityViewProps): JSX.Element {
  ...
  const checkConnection = async (): Promise<void> => {
    ...
    try {
      ...
      setMessage(`Owner reports ${health.status}. The token remains memory-only for this tab.`);
    } catch (error: unknown) {
      setConnection("error");
  ...
  return <section className="view-stack" aria-labelledby="compatibility-title">
    ...
    <div className="settings-grid">
      ...
      <section className="panel-card"><div className="panel-title"><div><p className="eyebrow">Session credential</p><h2>Pairing token</h2></div><StatusBadge tone="muted">memory only</StatusBadge></div><label className="field-label">Bearer token<input type="password" value={token} onChange={(event) => setToken(event.target.value)} placeholder="Paste for this tab only" autoComplete="off" /></label><div className="control-grid"><button className="button button-secondary" onClick={() => void checkConnection()} disabled={connection === "checking"}>{connection === "checking" ? "Checking…" : "Check owner connection"}</button><button className="button button-quiet" onClick={() => { setToken(""); liveClient.setToken(undefined); setConnection("idle"); setMessage("Session material cleared from this tab."); }}>Clear session</button></div>{message ? <p className={`connection-message connection-${connection}`}>{message}</p> : null}</section>
    </div>
