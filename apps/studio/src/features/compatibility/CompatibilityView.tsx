...
const admittedRoutes = [
  ...
  ["GET", "/v1/workflow-runs/:id/provider-session-policy", "pending"],
  ["POST", "/v1/workflow-runs/:id/provider-session-policy/import", "pending"],
  ["POST", "/v1/workflow-runs/:id/provider-session-policy/proposals/:id", "pending"],
  ["POST", "/v1/workflow-runs/:id/provider-session-policy/proposals/:id/approve", "pending"],
  ["POST", "/v1/workflow-runs/:id/provider-session-policy/proposals/:id/adopt", "pending"],
  ["POST", "/v1/workflow-runs/:id/provider-session-policy/adoptions", "pending"],
...
export function CompatibilityView({ mode, onModeChange, liveClient, linkMappings, onAddMapping, onRemoveMapping }: CompatibilityViewProps): JSX.Element {
  ...
  return <section className="view-stack" aria-labelledby="compatibility-title">
    ...
    <div className="settings-grid">
      ...
      <section className="panel-card"><div className="panel-title"><div><p className="eyebrow">Phase 1 admission</p><h2>Owner route matrix</h2></div><span className="muted">source: merged sts2-harness, plus pending PR #224</span></div><p className="muted">Saved provider-session policy routes are pending merge of Harness PR #224.</p><div className="route-table" role="table" aria-label="Phase 1 owner routes"><div className="route-row route-header" role="row"><span>Method</span><span>Path</span><span>Studio use</span></div>{admittedRoutes.map(([method, path, state]) => <div className="route-row" role="row" key={`${method}-${path}`}><code>{method}</code><code>{path}</code><StatusBadge tone={state === "admitted" ? "success" : "warning"}>{state}</StatusBadge></div>)}</div></section>
