...
import { useDraftConflict } from "./useDraftConflict";
...
export function DesignerView({ client, catalog, definition, initialDocument, initialRawText, mode, onBack, onRun, onRawTextChange }: DesignerViewProps): JSX.Element {
  ...
  const { draft, retrySave, persistedKeyRef, mergeBaseRef, mergeBaseLayoutRef, resetDraftForDocument } = persistence;

  const { publicationState, publish } = useDraftPublication({
    client,
    draftId,
    document,
    draft,
    setDraft: persistence.setDraft,
    editGeneration,
    setValidationState,
  ...
  const { conflictRemoteDocument, conflictRemoteLayout, reloadRemoteConflict, saveLocalAsNew, mergeConflict, cancelConflictResolution } = useDraftConflict({
    client,
    definitionId: definition.id,
    draftId,
    draft,
  ...
  useEffect(() => {
    ...
    setDiagnostics(undefined);
    setValidationState("idle");
    resetDraftForDocument(initialDocument, nextLayout);
  ...
  const validate = async (): Promise<void> => {
    ...
    try {
      ...
      if (persistedKeyRef.current === draftValueKey(document, layout)) {
        persistedKeyRef.current = draftValueKey(document, boundLayout);
      }
      setLayout(boundLayout);
    } catch (error: unknown) {
      setValidationState("error");
