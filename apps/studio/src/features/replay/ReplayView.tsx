...
import type { ReplayResponse, WorkflowDefinition } from "@studio/contracts";
...
export function ReplayView({ client, mode, definition }: ReplayViewProps): JSX.Element {
  ...
  const replay = async (): Promise<void> => {
    ...
    try {
      const [nextReplay, nextDiff] = await Promise.all([
        client.replay(runId),
        client.diff(definition, definition),
      ]);
  ...
  return <section className="view-stack" aria-labelledby="replay-title">
    ...
    <div className="replay-controls panel-card"><label className="field-label">Run ID<input value={runId} onChange={(event) => setRunId(event.target.value)} /></label><button className="button button-primary" onClick={() => void replay()} disabled={busy}>{busy ? "Comparing…" : "Replay offline"}</button><button className="button button-secondary" onClick={() => void downloadExport()}>Export redacted JSON</button></div>
